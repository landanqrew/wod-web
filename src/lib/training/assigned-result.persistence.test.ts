...
import { getClassSessionResultContext, getPRs, getResults } from "../data/training";
...
describe("Assigned Workout result ledger", () => {
  it("logs the athlete version through the shared PR and volume ledger, retaining history after leaving", async () => {
    ...
    const logged = await logAssignedWorkoutResultForAthlete(sessions[0].id, memberAthleteId, {
      performedAt: "2027-06-07T12:00:00Z",
      scoreType: ScoreType.Load,
      peakLoad: 155,
      rpe: 8,
      rx: true,
      movementResults: [{ movementId: "back_squat", reps: 5, load: 155, rx: true }],
    });
    ...
    await expect(
      logAssignedWorkoutResultForAthlete(sessions[0].id, memberAthleteId, {
    ...
    await expect(
      getUpcomingClassSessionsForAthlete(memberAthleteId, new Date("2027-06-01T00:00:00Z"))
    ).resolves.toEqual(
      expect.arrayContaining([
